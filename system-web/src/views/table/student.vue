    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="center" label-width="120px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="入学时间" prop="startTime">
          <el-date-picker v-model="temp.startTime" type="datetime" placeholder="请选择出生日期">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="手机号" prop="mobile">
          <el-input v-model.number="temp.mobile"></el-input>
        </el-form-item>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deletArticle, getGrade } from '@/api/article'
    ...
    return {
      ...
      rules: {
        timestamp: [{ type: 'date', required: true, message: 'timestamp is required', trigger: 'change' }],
         name: [
            { required: true,  trigger: 'blur',message:'请输入姓名',trigger:'change' }
          ],
        sex:[{required:true,message:'请选择性别',trigger:'change'}],
        age:[{required:true,message:'请输入年龄',trigger:'change'}],
        startTime:[{required:true,message:'请选择入学时间',trigger:'change'}],
        mobile:[{required:true,trigger:'change',validator:validPhone }],
        email:[{required:true,message:'请输入邮箱地址',trigger:'change'}],
        grade_id:[{required:true,message:'请选择班级',trigger:'change'}]
      },
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        this.list.forEach((item)=>{
          item.startTime = date_format(Number(item.start_time)*1000,'yyyy-MM-dd HH:mm:ss');
          if(item.sex == 0){
            item.sex = '男'
          }else{
            item.sex = '女'
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.temp.start_time = new Date(this.temp.startTime).valueOf();
          if(this.temp.sex == '男'){
            this.temp.sex = 0
          }else{
            this.temp.sex = 1
          ...
          createArticle(this.temp).then((res) => {
            ...
            this.list.forEach((item)=>{
              item.startTime = date_format(item.start_time,'yyyy-MM-dd HH:mm:ss');
              if(item.sex == 0){
                item.sex = '男'
              }else{
                item.sex = '女'
